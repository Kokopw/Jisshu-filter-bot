...
async def start(client:Client, message):
    ...
    if len(message.command) == 2 and message.command[1].startswith("reff_"):
        ...
        if fromuse == 100:
            ...
            # Reward the referrer with premium (was calling an undefined add_premium).
            expiry_time = dt.now() + timedelta(seconds=REF_PREMIUM * 24 * 60 * 60)
        ...
        try: 
         ...
         #       newPoint = await db.get_point(refUserId)
             if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        ...
                        reply_markup = InlineKeyboardMarkup(buttons)
                        m=await message.reply_sticker("CAACAgIAAxkBAAKJQGc08ZPovnO-yFab4-Bqc9f-bfiTAALHCwACDoNoSW0UzbcOaoH5NgQ") 
...
async def save_tutorial(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_caption https://t.me/CharlieSupportGroup</code>")    
...
async def set_tutorial_3(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_tutorial https://t.me/CharlieSupportGroup</code>")
...
async def refer(bot, message):
    btn = [[
        ...
        InlineKeyboardButton(f'⏳ {await referdb.get_refer_points(message.from_user.id)}', callback_data='ref_point'),
